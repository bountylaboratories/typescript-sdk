...
export interface ClientOptions {
  /**
   * Enter your API key (without "Bearer" prefix)
   */
   ...
   *
   * Defaults to process.env['BOUNTYLAB_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['BOUNTYLAB_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Bountylab API.
 */
export class Bountylab {
  apiKey: string;
   ...
   * API Client for interfacing with the Bountylab API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['BOUNTYLAB_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['BOUNTYLAB_BASE_URL'] ?? https://api.bountylab.io/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('BOUNTYLAB_BASE_URL'),
    apiKey = readEnv('BOUNTYLAB_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.BountylabError(
        "The BOUNTYLAB_API_KEY environment variable is missing or empty; either provide it, or instantiate the Bountylab client with an apiKey option, like new Bountylab({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.bountylab.io/v3`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Bountylab.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('BOUNTYLAB_LOG'), "process.env['BOUNTYLAB_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('BOUNTYLAB_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.bountylab.io/v3';
  }
  ...
  static Bountylab = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static BountylabError = Errors.BountylabError;

...
export class RawRepos extends APIResource {
  ...
  graph(
    ...
    return this._client.get(path`/api/raw/repos/${id}/graph/${relationship}`, { query, ...options });
  }
}
...
export interface RawRepoGraphParams {
  ...
  /**
   * Query param: Cursor for pagination (opaque base64-encoded string from previous
   * response)
   */
  after?: string;

  /**
   * Query param: Number of items to return (default: 100, max: 100)
   */
  first?: string;

  /**
   * Query param: Optional graph relationships to include (varies based on
   * relationship type)
   */
